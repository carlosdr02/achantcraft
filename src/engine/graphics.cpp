...
static void populateShaderStageCreateInfo(VkPipelineShaderStageCreateInfo& shaderStageCreateInfo, VkShaderStageFlagBits stage, VkShaderModule module) {
...
RayTracingPipeline::RayTracingPipeline(Device& device, uint32_t entryCount, const ShaderBindingTableEntry* entries, VkPipelineLayout pipelineLayout) {
    ...
    VkShaderModule* shaderModules = new VkShaderModule[shaderStageCount];
...
RayTracingPipeline::operator VkPipeline() {
    return pipeline;
}
...
void Renderer::recordCommandBuffers(VkDevice device, VkPipelineLayout pipelineLayout, RayTracingPipeline& rayTracingPipeline, VkExtent2D extent) {
